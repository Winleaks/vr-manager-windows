...
- Invoice PDFs generated from the normal authoritative invoice include a separate outstanding-invoices appendix. It spans all stores belonging to the same company and issuer, excludes cancelled/settled invoices, and uses cash payments, issued CN and applied credit to calculate remaining amounts. The current invoice appears once if outstanding. No due dates are invented: the Hub currently has no authoritative due-date field for this report.
...
- The weekly dashboard uses Monday–Sunday for issuance, payments and CN. Its outstanding card is explicitly labelled current balance across all history, not historical closing balance.
