...
export async function updateInvoiceDriveDocument(drive: any, input: {
  ...
  for (const file of response.data.files || []) {
    if (!validId(file.id || '') || !names.includes(file.name)) throw new InvoiceDriveDocumentError('Identitatea PDF-ului nu poate fi verificată.');
    if (!await inInvoiceTree(file.parents || [])) continue;
  ...
  const name = existing?.name || names[0];
  ...
  if (existing) {
    ...
    if (current.trashed || current.mimeType !== 'application/pdf' || current.name !== name ||
    ...
    const updated = await drive.files.update({ fileId: existing.id, media, ...move, fields: 'id', supportsAllDrives: true }, options);
