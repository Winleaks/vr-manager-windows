  ...
  return <div className="fixed inset-0 z-[300] bg-slate-950/70 flex items-center justify-center p-5" role="dialog" aria-modal="true">
    <form onSubmit={submit} className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl space-y-4">
      ...
      {fields.map((field, index) => <label key={field.name} className="block text-sm font-semibold">{field.label}{field.type === 'textarea'
        ...
        : <input autoFocus={index === 0} value={values[field.name]} type={field.type || 'text'} step={field.type === 'number' ? '0.01' : undefined} placeholder={field.placeholder} onChange={(event) => setValues((current) => ({ ...current, [field.name]: event.target.value }))} className="mt-1 w-full rounded-xl border p-3 font-normal" />}</label>)}
...
function ProtectedAccess({ onUnlocked, status }: AccessProps) {
  ...
  return <div className="min-h-screen bg-slate-950 flex items-center justify-center p-6">
    <div className="max-w-md w-full rounded-3xl bg-white p-8 shadow-2xl space-y-5">
      ...
      <label className="block text-sm font-semibold text-slate-700">{recoveryMode ? 'PIN nou' : status.configured ? 'PIN' : 'PIN nou'}<input type="password" inputMode="numeric" maxLength={6} value={pin} onChange={(event) => setPin(event.target.value.replace(/\D/g, '').slice(0, 6))} className="mt-1 w-full rounded-xl border border-slate-300 px-3 py-3 text-center text-xl tracking-[.5em]" autoFocus /></label>
      {(!status.configured || recoveryMode) && <label className="block text-sm font-semibold text-slate-700">Confirmă PIN-ul<input type="password" inputMode="numeric" maxLength={6} value={confirmation} onChange={(event) => setConfirmation(event.target.value.replace(/\D/g, '').slice(0, 6))} className="mt-1 w-full rounded-xl border border-slate-300 px-3 py-3 text-center text-xl tracking-[.5em]" /></label>}
...
function DashboardPanel() {
  ...
  useEffect(() => { api.protectedRegistry.getOverview().then(setData).catch((error) => window.alert(errorMessage(error))); }, []);
...
function ClientsPanel() {
  ...
  const toggle = async (company: any) => {
    ...
    catch (error) { window.alert(errorMessage(error)); } finally { setBusy(null); }
  };
...
function OrdersPanel() {
  ...
  const load = async () => {
    ...
    catch (error) { window.alert(errorMessage(error)); } finally { setBusy(false); }
  };
  const issue = async () => {
    if (!selected.length || !window.confirm(`Emiți ${selected.length} facturi în registrul separat?`)) return;
    setBusy(true);
    try { const result = await api.protectedRegistry.createWeekly(startDate, endDate, selected, operationId()); const failed = result.pdfResults.filter((row: any) => !row.success); window.alert(failed.length ? `Facturile au fost emise. ${failed.length} PDF-uri trebuie regenerate.` : `${result.invoices.length} facturi au fost emise și verificate în Drive.`); await load(); }
    catch (error) { window.alert(errorMessage(error)); } finally { setBusy(false); }
  };
  const issueZone = async (zoneId: string, rows: any[]) => {
    ...
    if (!ready.length || !window.confirm(`Emiți ${ready.length} facturi pentru zona ${zoneName}?`)) return;
    setBusy(true);
    try {
      ...
      window.alert(failed.length ? `Lotul a fost emis. ${failed.length} PDF-uri trebuie regenerate.` : `${result.invoices.length} facturi au fost emise pentru ${zoneName}.`);
      await load();
    } catch (error) { window.alert(errorMessage(error)); } finally { setBusy(false); }
  };
  const issueAll = async () => {
    const ready = (preview?.ordersByStore || []).filter((row: any) => row.billingState === 'ready');
    if (!ready.length || !window.confirm(`Emiți toate cele ${ready.length} facturi pregătite din registrul separat?`)) return;
    setBusy(true);
    try {
      const result = await api.protectedRegistry.createAllWeekly(startDate, endDate, operationId());
      const failed = result.pdfResults.filter((row: any) => !row.success);
      window.alert(failed.length ? `Lotul a fost emis. ${failed.length} PDF-uri trebuie regenerate.` : `${result.invoices.length} facturi au fost emise.`);
      await load();
    } catch (error) { window.alert(errorMessage(error)); } finally { setBusy(false); }
  };
...
function InvoicesPanel() {
  ...
  useEffect(() => { void reload(); }, [reload]);
  const cancel = async (invoice: any) => { const values = await ask({ title: `Anulează ${invoice.reference}`, fields: [{ name: 'reason', label: 'Motivul anulării', type: 'textarea' }] }); if (!values) return; try { await api.protectedRegistry.cancelInvoice(invoice.id, values.reason, operationId()); await reload(); } catch (error) { window.alert(errorMessage(error)); } };
  const remove = async (invoice: any) => { const values = await ask({ title: `Șterge definitiv ${invoice.reference}`, message: `Scrie exact STERGE ${invoice.reference}`, fields: [{ name: 'confirmation', label: 'Confirmare' }] }); if (!values) return; try { await api.protectedRegistry.deleteTestInvoice(invoice.id, values.confirmation, operationId()); await reload(); } catch (error) { window.alert(errorMessage(error)); } };
  const open = async (invoice: any) => { try { await api.protectedRegistry.openDocument('invoice', invoice.id); } catch (error) { window.alert(errorMessage(error)); } };
  const share = async (invoice: any) => { try { await api.protectedRegistry.shareDocument('invoice', invoice.id); } catch (error) { window.alert(errorMessage(error)); } };
  const reissue = async (invoice: any) => { if (!window.confirm(`Reemiți ${invoice.reference} cu un număr nou și emitentul actual al clientului?`)) return; try { const result = await api.protectedRegistry.reissueInvoice(invoice.id, operationId()); window.alert(result.pdf.success ? `Factura ${result.invoice.reference} a fost reemisă.` : `Factura ${result.invoice.reference} a fost reemisă, dar PDF-ul trebuie regenerat.`); await reload(); } catch (error) { window.alert(errorMessage(error)); } };
...
function PaymentsPanel() {
  ...
  useEffect(() => { void reload(); }, [reload]);
  const save = async () => { try { await api.protectedRegistry.recordPayment({ companyId: Number(companyId), issuerCode, amount: Number(amount), paymentDate: localToday(), method, operationId: operationId() }); setAmount(''); await reload(); } catch (error) { window.alert(errorMessage(error)); } };
  const reverse = async (payment: any) => { const values = await ask({ title: 'Reversează încasarea', fields: [{ name: 'reason', label: 'Motivul reversării', type: 'textarea' }] }); if (!values) return; try { await api.protectedRegistry.reversePayment(payment.id, values.reason, operationId()); await reload(); } catch (error) { window.alert(errorMessage(error)); } };
...
function CreditNotesPanel() {
  ...
  const toggle = (invoice: any, item: any) => {
    setSelected((current) => {
      ...
      if (anchor && (anchor.companyKey !== invoice.companyKey || anchor.issuerCode !== invoice.issuerCode)) {
        window.alert('Un Credit Note poate reuni numai facturi ale aceleiași companii și aceluiași emitent.'); return current;
      }
  ...
  const issue = async () => {
    if (!reason.trim() || !Object.keys(selected).length) return window.alert('Selectează pozițiile și completează motivul.');
    ...
    try {
      ...
      window.alert(result.pdf.success ? `Credit Note ${result.creditNote.reference} a fost emis și salvat în Drive.` : `Credit Note-ul a fost emis, dar PDF-ul trebuie regenerat: ${result.pdf.error}`);
...
function ExportsPanel() {
  ...
  const run = async () => {
    setBusy(true);
    try { const result = await api.protectedRegistry.exportMonth(month); window.alert(`Export verificat în Drive: ${result.excelName}, ${result.pdfCount} PDF-uri.`); }
    catch (error) { window.alert(errorMessage(error)); } finally { setBusy(false); }
  };
